...
--- Get normalized visual selection such that it returns the start_pos < end_pos
...
---
---@param prompt_messages { role: string, prompt_template: Path, args: table }
---@param make_job_fn fun(rendered_message: { role: string, content: string }, writer_fn: fun(content: string), completed_callback_fn: fun())
function M.invoke_llm_replace_mode(prompt_messages, make_job_fn)
  ...
  local visual_lines = api.nvim_buf_get_text(buf_id, srow - 1, scol - 1, erow - 1, ecol, {})
  ...
  vim.ui.input({ prompt = 'prompt: ' }, function(input)
    if input ~= nil then
      ...
      stream_end_extmark_id = api.nvim_buf_set_extmark(buf_id, kznllm_ns_id, erow - 1, ecol, {})

      -- wipe the range (preserving the extmark)
      api.nvim_buf_set_text(buf_id, srow - 1, scol - 1, erow - 1, ecol, {})
      ...
      for _, message in ipairs(prompt_messages) do
        table.insert(rendered_messages, { role = message.role, content = make_prompt_from_template(message.prompt_template, prompt_args) })
      end
